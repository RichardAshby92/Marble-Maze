...
bool CollisionDetection::RayBoxIntersection(const Ray&r, const Vector3& boxPos, const Vector3& boxSize, RayCollision& collision) {
	...
	if (bestT < 0.0f) { //for backwards rays
		return false;
	}

	Vector3 intersection = rayPos + (rayDir * bestT);
	const float epsilon = 0.0001f; //calc leeway
	for (int i = 0; i < 3; ++i) {
...
bool CollisionDetection::RaySphereIntersection(const Ray&r, const Transform& worldTransform, const SphereVolume& volume, RayCollision& collision) 
{
	...
	if (sphereProj < 0.0f) { return false; } //false when point is behind ray
...
bool CollisionDetection::SphereOBBIntersection(
	const OBBVolume& volumeA, const Transform& worldTransformA, 
	const SphereVolume& volumeB, const Transform& worldTransformB, CollisionInfo& collisionInfo) {

	//rotate OBB to AABB
	Quaternion orientation = worldTransformA.GetOrientation();
	Vector3 localPosition = worldTransformB.GetPosition() - worldTransformA.GetPosition();
	Vector3 transform = orientation.Conjugate() * localPosition;

	Vector3 boxASize = volumeA.GetHalfDimensions();
	Vector3 boxAPos = transform;

	Vector3 closestPointOnBox = Maths::Clamp(transform, -boxASize, boxASize);
	Vector3 localPoint =  transform - closestPointOnBox;
	float distance = localPoint.Length();

	if (distance < volumeB.GetRadius()) {
		Vector3 localCollisionNormal = localPoint.Normalised();
		float penetration = (volumeB.GetRadius() - distance);

		Vector3 localA = Vector3();
		//Vector3 localB = -localCollisionNormal * volumeB.GetRadius();

		Vector3 collisionNormal = (orientation * localCollisionNormal);
		Vector3 A = (orientation * localA) + worldTransformA.GetPosition();
		Vector3 B = -collisionNormal * volumeB.GetRadius();

		collisionInfo.AddContactPoint(A, B, collisionNormal, penetration);
...
bool CollisionDetection::OBBAABBIntersection(
	...
	Quaternion orientation = worldTransformA.GetOrientation();
	Vector3 localPosition = worldTransformA.GetPosition() - worldTransformB.GetPosition();
	Vector3 transform = orientation.Conjugate() * localPosition;
	...
	if (overlap) {
		...
		Vector3 localCollisionNormal = localPoint.Normalised();
		Vector3 collisionNormal = (orientation * localCollisionNormal);
		Vector3 A = (orientation * Vector3()) + worldTransformA.GetPosition();


		collisionInfo.AddContactPoint(A, Vector3(), bestAxis, penetration);
		return true;
	}
