...
void PhysicsSystem::ImpulseResolveCollision(GameObject& a, GameObject& b, CollisionDetection::ContactPoint& p) const {
	...
	Vector3 tangentVector = contactVelocity - p.normal.operator*((Vector3::Dot(contactVelocity, p.normal)) );
	float frictionForce = Vector3::Dot(tangentVector.Normalised(), contactVelocity);
	...
	Vector3 frictionIntertiaA = Vector3::Cross(physA->GetInertiaTensor() * Vector3::Cross(relativeA, tangentVector.Normalised()), relativeA);
	Vector3 frictionIntertiaB = Vector3::Cross(physB->GetInertiaTensor() * Vector3::Cross(relativeB, tangentVector.Normalised()), relativeB);
	float frictionAngularEffect = Vector3::Dot(frictionIntertiaA + frictionIntertiaB, tangentVector.Normalised());

	//Multiple phys objects Resitutions
	float AcResitution = physA->GetResitution();
	float BcResitution = physB->GetResitution();

	float jA = (-(1.0f + AcResitution) * impulseForce) / (totalMass + angularEffect);
	float jB = (-(1.0f + BcResitution) * impulseForce) / (totalMass + angularEffect);

	float jtA = -(physA->GetFriction() * frictionForce) / (totalMass + frictionAngularEffect);
	float jtB = (physB->GetFriction() * frictionForce) / (totalMass + frictionAngularEffect);

	Vector3 AfullImpulse = (p.normal * jA) -(tangentVector.Normalised() * jtA);
	Vector3 BfullImpulse = (p.normal * jB) - (tangentVector.Normalised() * jtB);

	//Add friction

	physA->ApplyLinearImpulse(-AfullImpulse);
	physB->ApplyLinearImpulse(BfullImpulse);

	physA->ApplyAngularImpulse(Vector3::Cross(relativeA, -AfullImpulse));
	physB->ApplyAngularImpulse(Vector3::Cross(relativeB, BfullImpulse));
}
...
void PhysicsSystem::BroadPhase() {
	...
	tree.OperateOnContents(
		[&](std::list<QuadTreeEntry<GameObject*>>& data) {CollisionDetection::CollisionInfo info;
			for (auto i = data.begin(); i != data.end(); ++i) {
				for (auto j = std::next(i); j != data.end(); ++j) {
					info.a = min((*i).object, (*j).object);
					info.b = min((*i).object, (*j).object);
...
void PhysicsSystem::IntegrateAccel(float dt) {
	...
	for (auto i = first; i != last; ++i) {
		...
		if (object == nullptr) { continue; } //no physics
		...
		if (applyGravity && inverseMass > 0) { accel += gravity; } //check for infinite weight
